...
def category_score(record: PlayerTeamSeason, category: Category) -> int:
    scorers = {
        ...
        Category.ALL_NBA_THIRD: lambda: 20 if record.all_nba == 3 else 0,
        Category.ALL_NBA_SECOND: lambda: 30 if record.all_nba == 2 else 0,
        Category.ALL_NBA_FIRST: lambda: 40 if record.all_nba == 1 else 0,
        Category.CHAMPION: lambda: 25 if record.champion else 0,
        Category.ALL_DEFENSE_SECOND: lambda: 30 if record.all_defense == 2 else 0,
